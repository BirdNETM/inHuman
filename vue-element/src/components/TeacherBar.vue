<template>
    ...
    >
      <el-sub-menu v-for="item in menuData" :key="item.index" :index="item.index">
        ...
        <el-menu-item
          ...
          :key="subItem.index"
          :index="subItem.index"
          :route="{ path: `/TeacherHome/${subItem.index}` }"
        >
          {{ subItem.title }}
        </el-menu-item>
      </el-sub-menu>
  ...
  export default {
    setup() {
      ...
      if (currentPath.includes('/TeacherHome/1-1')) {
        activeMenu.value = '1-1';
      } else if (currentPath.includes('/TeacherHome/1-2')) {
        activeMenu.value = '1-2';
      } else if (currentPath.includes('/TeacherHome/2-1')) {
        activeMenu.value = '2-1';
      } else if (currentPath.includes('/TeacherHome/2-2')) {
        activeMenu.value = '2-2';
      } else if (currentPath.includes('/TeacherHome/3-1')) {
